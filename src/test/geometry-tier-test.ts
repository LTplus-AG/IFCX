...
import { ifcToTiered, IfcLiteUnavailableError } from "../ifcx-cli/ifc-to-tiered";
import { evaluateBSplineCurve, sampleBSplineCurve, evaluateBSplineSurface } from "../ifcx-core/geometry/nurbs";
import { canonicalize, sourceHashOf, sha256Hex } from "../ifcx-core/geometry/source-hash";
...
describe("tier resolver", () => {
    ...
    it("viewer config never reads the procedural table", () => {
        const provider = new InMemoryTableProvider()
            .addTable(makeMeshTable())
            .addTable(makeProcTable());
        const resolver = new TierResolver(provider, ["mesh"]);
...
describe("source-hash canonicalization", () => {
    ...
    it("matches SHA-256 known-answer vectors, including a multi-block input", () => {
        expect(sha256Hex("")).to.equal("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
        expect(sha256Hex("abc")).to.equal("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
        // 100 bytes > 64 exercises the multi-block padding path
        expect(sha256Hex("a".repeat(100))).to.equal("2816597888e4a0d3a36b82b83316ab32680eb8f00f8cd3b904d681246d285a0e");
    });
});
...
describe("tiered example file", () => {
    ...
    it("phase 6 — ifcToTiered converts hello-wall.ifc, preserving IFC GUIDs as paths", async () => {
        ...
        try {
            ...
            let result;
            try {
                result = await ifcToTiered(
                    `${examplesFolderPath}/Hello Wall/hello-wall.ifc`,
                    tmpDir,
                );
            } catch (e) {
                if (e instanceof IfcLiteUnavailableError) {
                    console.log("\x1b[33m [skip] phase 6 — ifc-lite backend unavailable (set IFC_LITE_DIST to enable) \x1b[0m");
                    return;
                }
                throw e;
            }
